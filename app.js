...
短袖 襯衫'},
  {type:'下身',src:'assets/girl-pants.webp',name:'黑色長褲',detail:'褲長及腳踝<br><span class="choice">長褲、長裙擇一</span>',query:'女 黑色 長褲',variants:[
...
/g,' ')+' '+brand)}
function link(query,brand){const label=brand||'不限品牌';const cls=brand==='GU'?'gu':brand==='UNIQLO'?'uniqlo':!brand?'other':brand==='La New'||brand==='阿瘦'?'shoe':'';return `<a class="button ${cls}" data-brand="${brand}" target="_blank" rel="noopener noreferrer" aria-label="搜尋 ${query.replace(/
...
document.querySelector('#main').innerHTML=displayGroups.map(group=>`<section class="group ${group.cls}" aria-labelledby="${group.cls}-title">
 ...
 <div class="cards">${group.items.map((item,index)=>`<article class="card" data-item="${index}"><h3>${item.type}</h3><div class="photo"><img src="${item.variants?item.variants[0].src:item.src}" alt="${item.name}款式示意">${group.cls==='girl'&&item.type==='襪子'?'<span class="sock-or" aria-hidden="true">或</span>':''}${item.variants?'<span class="photo-status" role="status" aria-live="polite" hidden></span>'+variantButtons('photo'):''}</div><div class="desc"><b>${item.name}</b><small>${item.detail}</small></div></article>`).join('')}</div>
 <div class="search"><h3 class="section">${art('search-'+group.cls,'section-icon')}<span>建議搜尋關鍵字 <small>（點擊按鈕即可搜尋）</small></span></h3><div class="searchgrid">${group.items.map((item,index)=>`<div class="searchcard" data-item="${index}"><div class="search-head">${item.variants?variantButtons('search'):item.type}</div><div class="keywords">${item.query}</div>${(group.cls==='boy'&&index===3?['','La New','阿瘦']:brands).map(brand=>link(item.query,brand)).join('')}</div>`).join('')}</div></div>
...
function setLowerVariant(index){
  ...
  incoming.onerror=()=>{ status.textContent='照片暫時無法載入，請再點選一次。'; };
}
